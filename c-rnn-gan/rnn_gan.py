...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session(config=tf.ConfigProto(log_device_placement=FLAGS.log_device_placement)) as session:
	...
	# Gráficos test
        ...
        with open(os.path.join(plots_dir, 'tiempos.txt'), 'a') as f:
          f.write(f"Etapa {i}: {elapsed_time} segundos\n")



        """end_time = time.time()
        print('Ended time: '+str(end_time))
        if lista_tiempos_ejecucion == []:
          lista_tiempos_ejecucion.append(end_time-start_time)
        else: 
          ultima_iteracion = end_time - lista_tiempos_ejecucion[-1]
          print(ultima_iteracion)
          lista_tiempos_ejecucion.append(ultima_iteracion)
        with open(os.path.join(plots_dir, 'tiempos.txt'), 'a') as f:
