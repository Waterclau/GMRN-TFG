...
export class GeneratorComponent implements OnInit {
 ...
 // src\assets\music\generated\one.mp3
  ...
  ngOnInit(): void {
    ...
    this.idGen = Math.floor(Math.random() * 100) + 1;
  }


  goToLibrary() {
    if (this.loginService.loginUser == true) {
      this.router.navigate(['/pages/tables/biblioteca']); // Redireccionar a la URL deseada
  ...
  goToRegister(){
    this.router.navigate(['auth/register']); // Redireccionar a la URL deseada
